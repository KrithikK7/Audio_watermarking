...
def ensure_log_header(log_path: str) -> None:
    ...
    if not contents:
        return
    header = contents[0]
    if "outfile_sha256" in header.split(","):
        return
    upgraded: List[str] = []
    for idx, line in enumerate(contents):
        if idx == 0:
            upgraded.append(line + ",outfile_sha256")
        elif line.strip():
...
def embed_frames_audioseal(audio_np: np.ndarray, orig_sr: int, frames_with_pilot: List[List[int]],
    ...
    for fr in frames_with_pilot:
        ...
        seg_batch = seg.unsqueeze(0).unsqueeze(0)  # AudioSeal expects [B, C, T]
        ...
        try:
            ...
            # Some AudioSeal builds expect the legacy positional order (audio, sample_rate, message)
            wm_seg = gen.get_watermark(seg_batch, AS_SR, message)
        wm_seg_1d = wm_seg.squeeze()  # collapse [B,C,T] back to mono
...
def main():
    ...
    with open(args.log, "a", newline="", encoding="utf-8") as f:
        ...
        with open(outfile, "rb") as fout:
            outfile_sha256 = sha256_hex(fout.read())
        w.writerow({
