#!/usr/bin/env python3
"""
wm_singlepath.py — Single‑path audio watermark embedder/decoder (v7.7)
...
def strip_pilot_frames(frames_with_pilot: list, sync_every: int, pilot_bits: int) -> list:
    """Mirror insertion: pilots = ceil(pilot_bits/16) frames, then (sync_every-1) data frames, repeating."""
    pilot_frames = max(1, (pilot_bits + 15) // 16)
    data_frames = []
    i = 0
    n = len(frames_with_pilot)
    while i < n:
        # skip pilot frames
        i += pilot_frames
        # then collect data frames until next pilot
