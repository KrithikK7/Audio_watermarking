...
def bch63_encode_bits(bits: List[int]) -> List[int]:
    ...
    out_bits = []
    # Pack 45 data bits into 6 bytes (ceil) chunk by chunk
    i = 0
    while i < len(bits):
        block = bits[i:i+45]
        if len(block) < 45:
        ...
        for b in block:
            d = (d<<1) | (b&1)
        data_len = math.ceil(45/8)
        data_bytes = d.to_bytes(data_len, "big")
        ...
        for by in packet:
            for k in range(7,-1,-1):
                pb.append((by>>k)&1)
        out_bits.extend(pb[:63])
        i += 45
    # pad to multiple of 16 for framing
    pad = (-len(out_bits)) % 16
    out_bits += [0]*pad
    return out_bits
    ...
    while i+63 <= len(bits):
        ...
        for j in range(0,63,8):
            ...
            for b in chunk:
                val = (val<<1) | (b&1)
            bys.append(val)
        data, ecc = bys[:-bch.ecc_bytes], bys[-bch.ecc_bytes:]
        d, _ = bch.decode(data, ecc)
        # back to 45 bits
        val = int.from_bytes(d, "big")
        for k in range(45-1, -1, -1):
            out.append((val>>k)&1)
        i += 63
    return out
