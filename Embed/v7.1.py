#!/usr/bin/env python3
"""
wm_singlepath.py — Single‑path audio watermark embedder/decoder (v7.5)
...
auth_key = Ka if args.secret else ((args.auth_key or "").encode("utf-8"))
    ...
    if args.mode == "selftest":
        ...
        # (Since we know the pilot schedule, just remove pilot frames by counting cadence.)
        pilot_frames = math.ceil(max(1, args.pilot_bits) / 16)
        data_frames = []
        cnt = 0
        for fr in frames_with_pilot:
            if cnt % (args.sync_every + 1) == 0:
                # This and subsequent (pilot_frames-1) frames are pilot; skip them
                cnt += 1
                # fast-forward through remaining pilot frames
                for _ in range(pilot_frames - 1):
                    cnt += 1
                continue
        ...
        raw_bits2 = remove_fec(deint_bits, args.fec)
